...
| **Animated SVG Download** | `LogoCard.tsx` → "↓ Animated SVG" | ✅ The concept's animation as a file that plays itself, for concepts registered in `logos/animatedMarks.ts` (25, 33, 34, 36, 41 — every final nominee). A concept whose animation is stated as pieces seating into place needs only to hand them to `logos/seatedMark.ts`; one absent from the registry shows no animated export at all. The file follows the version the card is showing: the mark alone, or the whole lockup with the company name standing beside the mark as it assembles |
